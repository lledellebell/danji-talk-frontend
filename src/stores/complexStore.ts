...
export const useComplexStore = create<ComplexState>((set, get) => ({
  ...
  fetchComplexes: async () => {
    try {
      ...
      const response = await axios.get('https://danjitalk.duckdns.org/api/apartment');
  ...
  fetchComplexById: async (id: number) => {
    try {
      set({ isLoading: true, error: null });
      const response = await axios.get(`https://danjitalk.duckdns.org/api/apartment/${id}`);
  ...
  searchComplexes: async (keyword: string) => {
    try {
      set({ isLoading: true, error: null });
      const response = await axios.get(`https://danjitalk.duckdns.org/api/apartment/search?keyword=${encodeURIComponent(keyword)}`);
      set({ complexes: response.data, isLoading: false });
    } catch (error) {
