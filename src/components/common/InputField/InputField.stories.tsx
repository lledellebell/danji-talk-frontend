...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: '사용자 입력을 받는 기본 입력 필드 컴포넌트입니다. 다양한 상태와 유효성 검사를 지원합니다.',
      },
    },
  },
  tags: ['autodocs'],
  argTypes: {
    ...
    showPasswordToggle: {
      description: '비밀번호 표시/숨기기 토글 버튼 표시 여부 (type이 password인 경우에만 적용)',
      control: 'boolean',
      table: {
        type: { summary: 'boolean' },
        defaultValue: { summary: false },
    ...
    actionButton: {
      ...
      table: {
        type: { 
          summary: '{ label: string; onClick: () => void; disabled?: boolean; }',
          detail: `{
...
const ControlledInput = (props: Partial<React.ComponentProps<typeof InputField>>) => {
...
const ControlledPasswordInput = (props: Partial<React.ComponentProps<typeof InputField>>) => {
  const [value, setValue] = useState("");
  return (
    <InputField
      label="비밀번호"
...
const ControlledInputWithAction = (props: Partial<React.ComponentProps<typeof InputField>>) => {
  const [value, setValue] = useState("");
...
export const WithHelperText: Story = {
  render: () => <ControlledInput helperText="example@email.com 형식으로 입력해주세요" />,
};
